...
- [2026-03-12] - Registration Role Assignment: Never use `.update()` to set the user role in `user_profiles` after `supabase.auth.signUp()`. The DB trigger that creates the profile row may not have fired yet, causing `.update()` to silently match zero rows and leave the role as default `student`. Always use `.upsert({ ... }, { onConflict: 'id' })` with a short delay. Also always set BOTH the `role` text column AND the boolean flags (`is_admin`, `is_tutor`, `is_student`) since `RoleProtectedRoute` checks boolean flags first.
- [2026-03-12] - Supabase Auth Client Deadlock: Never run `await supabase.from(...)` directly inside `onAuthStateChange` listeners when using standard `@supabase/supabase-js` setups. The Postgrest client internally requests `getSession()` to attach the Auth header, which attempts to acquire the `localStorage` auth lock. However, if the listener was triggered by `signInWithPassword`, that function already holds the lock, causing a browser hang/deadlock. Always wrap database calls inside `onAuthStateChange` in a `setTimeout(async () => { ... }, 0)` to escape the synchronous event emitter queue.
