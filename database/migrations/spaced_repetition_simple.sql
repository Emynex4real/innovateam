-- Spaced Repetition System - SIMPLIFIED VERSION
-- Run this if you don't have ai_questions table yet
...
CREATE INDEX IF NOT EXISTS idx_mastery_student ON student_question_mastery(student_id);
CREATE INDEX IF NOT EXISTS idx_mastery_next_review ON student_question_mastery(student_id, next_review_date);
...
BEGIN
  ...
  IF p_is_correct THEN
    IF v_mastery.consecutive_correct = 0 THEN 
      v_new_interval := 1;
    ELSIF v_mastery.consecutive_correct = 1 THEN 
      v_new_interval := 6;
    ELSE 
      v_new_interval := ROUND(v_mastery.interval_days * v_new_ease);
    END IF;
  ELSE
    v_new_interval := 1;
  END IF;
...
DROP POLICY IF EXISTS student_mastery_policy ON student_question_mastery;
CREATE POLICY student_mastery_policy ON student_question_mastery 
  FOR ALL 
  USING (student_id = auth.uid());

COMMENT ON TABLE student_question_mastery IS 'Tracks question mastery using spaced repetition (SuperMemo-2)';
