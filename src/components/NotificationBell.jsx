...
const NotificationBell = () => {
  ...
  useEffect(() => {
    ...
    // Pause polling during exams
    const handleExamStart = () => clearInterval(interval);
    const handleExamEnd = () => fetchNotifications();
    window.addEventListener("exam-mode-start", handleExamStart);
    window.addEventListener("exam-mode-end", handleExamEnd);

    return () => {
      clearInterval(interval);
      window.removeEventListener("exam-mode-start", handleExamStart);
      window.removeEventListener("exam-mode-end", handleExamEnd);
    };
  }, []);
                ...
                <div className="divide-y divide-gray-100">
                  {notifications.map((notif) => (
                    <div
                      key={notif.id}
                      onClick={() =>
                        handleMarkAsRead(notif.id, notif.action_url)
                      }
