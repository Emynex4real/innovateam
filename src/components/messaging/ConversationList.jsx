...
const ConversationList = ({
  ...
  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 30000); // 30s instead of 5s
  ...
  const filtered = conversations.filter((c) => {
    const name =
      c.partnerName || c.other_user?.full_name || c.other_user?.email || "";
    return name.toLowerCase().includes(search.toLowerCase());
  });

  const getDetails = (c) => ({
    name:
      c.partnerName || c.other_user?.full_name || c.other_user?.email || "User",
    avatar: c.partnerAvatar || c.other_user?.avatar_url,
    initial: (c.partnerName || c.other_user?.email || "U")[0].toUpperCase(),
    time: c.lastMessageTime
      ? new Date(c.lastMessageTime).toLocaleDateString(undefined, {
          month: "short",
          day: "numeric",
        })
      : "",
