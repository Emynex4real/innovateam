...
const StudentProfile = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load parent WhatsApp from students list
        try {
          const studentsRes = await tutorialCenterService.getStudents();
          if (studentsRes.success) {
            const match = studentsRes.students.find((s) => s.id === studentId);
            if (match?.parent_whatsapp)
              setParentWhatsapp(match.parent_whatsapp);
          }
        } catch {}
      } catch (error) {
  ...
  const handleSaveWhatsapp = async () => {
    ...
    try {
      await tutorialCenterService.updateParentWhatsapp(
        studentId,
        parentWhatsapp,
      );
      toast.success("Parent WhatsApp saved");
    } catch {
  ...
  const handleAddNote = async () => {
    ...
    try {
      const res = await tutorialCenterService.addStudentNote(
        studentId,
        newNote,
      );
      if (res.success) {
  ...
  const handleDownloadReport = async () => {
    ...
    try {
      const res = await tutorialCenterService.generateStudentReport(
        studentId,
        reportPeriod,
      );
      if (res.success) {
  ...
  return (
    ...
    >
      {/* 1. TOP NAVIGATION */}
      <div
        className={`sticky top-0 z-30 px-4 md:px-8 py-4 flex items-center justify-between border-b backdrop-blur-md ${isDarkMode ? "bg-gray-900/80 border-gray-800" : "bg-white/80 border-gray-200"}`}
      >
        ...
        <div className="flex gap-2">
          ...
          <button
            className={`p-2 rounded-xl border transition-colors ${isDarkMode ? "border-gray-700 hover:bg-gray-800 text-gray-400" : "border-gray-200 hover:bg-gray-50 text-gray-600"}`}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            >
              {/* Background Gradient */}
              <div
                className={`absolute top-0 left-0 right-0 h-32 bg-gradient-to-br from-green-600 to-green-700 opacity-10`}
              />

              <div className="relative flex flex-col items-center text-center">
                <div className="w-28 h-28 rounded-3xl bg-gradient-to-br from-green-600 to-green-700 p-1 mb-4 shadow-xl">
                  <div
                    className={`w-full h-full rounded-[20px] flex items-center justify-center text-3xl font-bold text-white ${isDarkMode ? "bg-gray-900" : "bg-white text-blue-600"}`}
                  >
                    {initials}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 w-full mb-8">
                  <div
                    className={`p-3 rounded-2xl ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}
                  >
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          className={`w-full pl-9 pr-3 py-2 rounded-lg text-sm border outline-none focus:ring-2 focus:ring-green-500 ${isDarkMode ? "bg-gray-800 border-gray-700 text-white" : "bg-gray-50 border-gray-200"}`}
                        />
                      </div>
                      <button
                        onClick={handleSaveWhatsapp}
                        disabled={savingWhatsapp}
                        className="px-3 py-2 bg-green-600 text-white rounded-lg text-xs font-bold hover:bg-green-700 transition disabled:opacity-50"
                      >
                        {savingWhatsapp ? "..." : "Save"}
                      </button>
                    </div>
                    <p className="text-xs text-gray-400 mt-1">
                      Used for one-click test result sharing
                    </p>
                  </div>
                </div>
              </div>
            ...
            >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div
                    className={`h-2 w-full rounded-full ${isDarkMode ? "bg-gray-800" : "bg-gray-100"}`}
                  >
                    <div
          ...
          <div className="lg:col-span-8">
            {/* Tabs */}
            <div
              className={`flex gap-2 p-1 rounded-2xl mb-8 overflow-x-auto ${isDarkMode ? "bg-gray-900/50" : "bg-white border border-gray-100"}`}
            >
              {[
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === "history" && (
                ...
                >
                  {testHistory.length === 0 ? (
                    <div
                      className={`text-center py-20 rounded-3xl border border-dashed ${isDarkMode ? "border-gray-800" : "border-gray-200"}`}
                    >
                    ...
                    testHistory.map((test) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <h4
                              className={`font-bold text-lg group-hover:text-blue-600 transition-colors ${isDarkMode ? "text-white" : "text-gray-900"}`}
                            >
              ...
              {activeTab === "notes" && (
                ...
                >
                  <div
                    className={`p-6 rounded-3xl border mb-8 ${isDarkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-100"}`}
                  >
                  ...
                  <div className="space-y-6 relative pl-6 border-l-2 border-gray-200 dark:border-gray-800 ml-4">
                    {notes.map((note) => (
                      <div key={note.id} className="relative">
                        <div
                          className={`absolute -left-[31px] top-0 w-4 h-4 rounded-full border-2 ${isDarkMode ? "bg-gray-900 border-blue-500" : "bg-white border-blue-500"}`}
                        />
                        <div
                          className={`p-5 rounded-2xl border ${isDarkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-100"}`}
                        >
