...
const Questions = () => {
  ...
  const selectAllInTopic = (subject, topic) => {
    ...
    const allSelected = topicQuestions.every((id) =>
      selectedQuestions.includes(id),
    );
    setSelectedQuestions((prev) =>
      allSelected
        ? prev.filter((id) => !topicQuestions.includes(id))
        : [...new Set([...prev, ...topicQuestions])],
    );
  };
  ...
  const selectAllInSubject = (subject) => {
    const subjectQuestions = Object.values(groupedQuestions[subject])
      .flat()
      .map((q) => q.id);
    const allSelected = subjectQuestions.every((id) =>
      selectedQuestions.includes(id),
    );
    setSelectedQuestions((prev) =>
      allSelected
        ? prev.filter((id) => !subjectQuestions.includes(id))
        : [...new Set([...prev, ...subjectQuestions])],
    );
  };
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      !window.confirm(
        `Delete ${selectedQuestions.length} questions? This cannot be undone.`,
      )
    )
      return;

    const toastId = toast.loading(
      `Deleting ${selectedQuestions.length} questions...`,
    );
    try {
      let deleted = 0;
      for (const id of selectedQuestions) {
        ...
        deleted++;
        if (deleted % 5 === 0) {
          toast.loading(`Deleting ${deleted}/${selectedQuestions.length}...`, {
            id: toastId,
          });
        }
      }
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const result = await tutorialCenterService.uploadQuestionImage(file);
      if (result.success) {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border font-medium transition-all ${isDarkMode ? "bg-blue-900/30 border-blue-800 text-blue-400 hover:bg-blue-900/50" : "bg-blue-50 border-blue-200 text-blue-700 hover:bg-blue-100"}`}
            >
              <Library size={18} />
        ...
        {/* Toolbar Section */}
        <div
          className={`rounded-xl border shadow-sm p-4 mb-6 ${isDarkMode ? "bg-gray-900 border-gray-800" : "bg-white border-gray-200"}`}
        >
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
            ...
            <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
              ...
              <div
                className={`flex rounded-lg border p-1 ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-gray-50 border-gray-200"}`}
              >
                <button
          ...
          {viewMode === "hierarchical" ? (
            <div className="space-y-4">
              ...
              {Object.keys(groupedQuestions).map((subject) => {
                const subjectQuestions = Object.values(
                  groupedQuestions[subject],
                ).flat();
                const selectedInSubject = subjectQuestions.filter((q) =>
                ...
                return (
                  <div
                    key={subject}
                    className={`rounded-xl border overflow-hidden transition-all ${isDarkMode ? "border-gray-800 bg-gray-900/50" : "border-gray-200 bg-white"}`}
                  >
                    {/* Subject Header */}
                    <div
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`p-2 rounded-lg ${isExpanded ? "bg-green-100 text-green-600 dark:bg-green-900/30 dark:text-green-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}
                        >
                          {isExpanded ? (
                            <FolderOpen size={20} />
                          ) : (
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`text-xs px-2 py-1 rounded font-medium ${selectedInSubject > 0 ? "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-500"}`}
                        >
                    ...
                    {isExpanded && (
                      ...
                      >
                        {Object.keys(groupedQuestions[subject]).map((topic) => {
                          ...
                          const selectedInTopic = topicQuestions.filter((q) =>
                            selectedQuestions.includes(q.id),
                          ).length;
                          ...
                          return (
                            <div
                              key={topic}
                              className={`border-b last:border-0 ${isDarkMode ? "border-gray-800" : "border-gray-100"}`}
                            >
                              <div
                                className={`flex items-center justify-between p-3 pl-6 md:pl-12 cursor-pointer transition-colors ${
                                  isDarkMode
                                    ? "hover:bg-gray-800/50 bg-gray-900"
                                    : "hover:bg-gray-50 bg-gray-50/30"
                                }`}
                                onClick={() => toggleTopic(subject, topic)}
                              >
                                <div className="flex items-center gap-3">
                                  <div
                                    className={`text-gray-400 transition-transform duration-200 ${isTopicExpanded ? "rotate-90" : ""}`}
                                  >
                                ...
                                <button
                                  ...
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    selectAllInTopic(subject, topic);
                                  }}
                              ...
                              {isTopicExpanded && (
                                <div
                                  className={`p-4 pl-6 md:pl-16 grid gap-4 ${isDarkMode ? "bg-gray-900" : "bg-gray-50"}`}
                                >
                                  {topicQuestions.map((q) => (
                                    <QuestionCard
                                      key={q.id}
                                      question={q}
                                      isSelected={selectedQuestions.includes(
                                        q.id,
                                      )}
                                      toggleSelect={() =>
                                        setSelectedQuestions((prev) =>
        ...
        {showForm && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div
              className={`${isDarkMode ? "bg-gray-900 border border-gray-800" : "bg-white"} rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl`}
            >
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar">
                ...
                >
                  ...
                  <div>
                    ...
                    {formData.image_url ? (
                      <div
                        className={`relative inline-block rounded-xl border-2 border-dashed p-2 ${isDarkMode ? "border-gray-700 bg-gray-800" : "border-gray-200 bg-gray-50"}`}
                      >
                        <img
                        ...
                        <div className="flex gap-2 mt-2">
                          <button
                            ...
                            className="text-xs px-3 py-1.5 bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300 rounded-lg font-medium hover:bg-blue-200 dark:hover:bg-blue-900/50 transition"
                          >
                            Change Image
                          </button>
                          <button
                            ...
                            className="text-xs px-3 py-1.5 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300 rounded-lg font-medium hover:bg-red-200 dark:hover:bg-red-900/50 transition"
                          >
                            Remove
                          </button>
                        </div>
                      ...
                      <button
                        ...
                        onClick={() => imageInputRef.current?.click()}
                        disabled={uploadingImage}
                        className={`flex items-center gap-2 px-4 py-3 rounded-xl border-2 border-dashed transition-all w-full justify-center ${isDarkMode ? "border-gray-700 bg-gray-800 hover:border-gray-600 text-gray-400" : "border-gray-200 bg-gray-50 hover:border-green-400 hover:bg-green-50 text-gray-500"} ${uploadingImage ? "opacity-60 cursor-wait" : ""}`}
                      >
                        {uploadingImage ? (
                          <>
                          ...
                          <>
                            <ImagePlus size={18} /> Add Image (diagram, graph,
                            map...)
                          </>
                        )}
                      </button>
                    ...
                    {["A", "B", "C", "D"].map((letter, idx) => (
                      <div key={letter}>
                        ...
                        <div className="flex items-center gap-2">
                          <span
                            className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${formData.correct_answer === letter ? "bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800"}`}
                          >
                            {letter}
                          </span>
                          <input
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                      ...
                      <select
                        ...
                        className={`w-full px-4 py-2 border rounded-lg ${isDarkMode ? "bg-gray-800 border-gray-700 text-white" : "bg-white border-gray-200"}`}
                      >
                        {["A", "B", "C", "D"].map((opt) => (
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className={`w-full px-4 py-3 border rounded-lg ${isDarkMode ? "bg-gray-800 border-gray-700 text-white" : "bg-white border-gray-200"}`}
                      rows="2"
                  ...
                  <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700">
                    <div>
                      ...
                      <input
                        ...
                        className={`w-full px-3 py-1.5 text-sm border rounded ${isDarkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"}`}
...
const QuestionCard = ({
  ...
  <div
    className={`group relative p-4 md:p-6 rounded-xl border transition-all ${
      isSelected
        ? isDarkMode
    ...
    <div className="flex items-start gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          {question.topic && (
            <span
              className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${isDarkMode ? "bg-gray-800 text-gray-300" : "bg-gray-100 text-gray-600"}`}
            >
              {question.topic}
            </span>
          )}
          <span
            className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
              (question.difficulty_level || question.difficulty) === "easy"
        ...
        {question.image_url && (
          <div className="mb-3">
            <img
              ...
              alt="Question"
              className={`max-h-[120px] rounded-lg border object-contain ${isDarkMode ? "border-gray-700" : "border-gray-200"}`}
            />
          </div>
        )}
        ...
        <div
          className={`text-sm p-3 rounded-lg ${isDarkMode ? "bg-gray-800" : "bg-gray-50"}`}
        >
          <span className="font-bold text-gray-500 text-xs uppercase mr-2">
            Explanation:
          </span>
  ...
  <div className="flex flex-col items-center justify-center py-16 px-4 text-center rounded-2xl border border-dashed border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
    <div className="w-16 h-16 bg-white dark:bg-gray-800 rounded-full flex items-center justify-center shadow-sm mb-4">
      {isSearch ? (
        ...
        <Brain size={32} className="text-green-500" />
      )}
    </div>
