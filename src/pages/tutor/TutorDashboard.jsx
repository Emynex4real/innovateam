...
const EnterpriseTutorDashboard = () => {
  ...
  const handleEdit = async () => {
    ...
    try {
      const res = await tutorialCenterService.updateCenter({
        name: editName.trim(),
  ...
  const handleDelete = async () => {
    try {
      const res = await tutorialCenterService.deleteCenter({
  ...
  const loadData = async (isMounted = true, loadId = "unknown") => {
    try {
      ...
      const [studentsRes, testsRes, attemptsRes, questionsRes] =
        await Promise.all([
          loadWithDelay(
          ...
          loadWithDelay(
            () => tutorialCenterService.getQuestions({ limit: 1000 }),
            "questions",
            600,
          ),
        ]);
      ...
      if (studentsRes.success) {
        ...
        const avgScore = students.length
          ? Math.round(
              students.reduce((sum, s) => sum + (s.average_score || 0), 0) /
                students.length,
            )
          : 0;
      ...
      if (questionsRes.success) {
        setStats((prev) => ({
          ...prev,
          questions: questionsRes.questions.length,
        }));
      }
      ...
      try {
        const limitsRes = await requestManager.deduplicate(
          "checkLimits",
          async () => {
        ...
        if (limitsRes?.success) {
          const { usage, limits: planLimits, graceInfo } = limitsRes;
          const warnings = [];
          ...
          ["students", "questions", "tests"].forEach((resource) => {
            ...
            if (
              planLimits?.[maxKey] &&
              usage?.[resource] >= planLimits[maxKey]
            ) {
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-8">
        ...
        >
          ...
          <p className="text-gray-600 dark:text-gray-400 mb-8">
            Set up your professional tutorial center to start creating
            assessments and tracking students.
          </p>
          <button
          ...
          <div className="flex items-center gap-4">
            {logoUrl ? (
              <img
                src={logoUrl}
                alt="Logo"
                className="w-16 h-16 object-contain rounded-xl bg-white p-2 shadow-sm border border-gray-100"
              />
            ) : (
            ...
            <div>
              ...
              <p className="text-gray-500 dark:text-gray-400 flex items-center gap-2 text-sm mt-1">
                <span className="bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 px-2 py-0.5 rounded-full font-medium text-xs">
                  Verified Center
                </span>
                • Code:{" "}
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {planWarnings.some(
                  (w) => !w.expired && w.daysRemaining !== undefined,
                ) && (
                  <div className="mt-2 p-2 bg-white/50 dark:bg-black/20 rounded-lg">
                    ...
                    {planWarnings
                      ...
                      .map((w) => (
                        <p
                          key={w.resource}
                          className="text-xs mt-1 text-amber-800 dark:text-amber-300"
                        >
                          <span className="font-semibold capitalize">
                            {w.resource}:
                          </span>{" "}
                          {w.daysRemaining} day
                          {w.daysRemaining !== 1 ? "s" : ""} remaining until{" "}
                          {new Date(w.graceEndsAt).toLocaleDateString()}
                        </p>
                      ))}
                  </div>
                )}
                ...
                {planWarnings.some((w) => w.expired) && (
                  <p className="text-sm mt-2 font-medium text-red-800 dark:text-red-300">
                    Your grace period has expired. You cannot create new{" "}
                    {planWarnings
                      .filter((w) => w.expired)
                      .map((w) => w.resource)
                      .join(" or ")}{" "}
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <h3 className="text-lg font-bold mb-1 group-hover:text-amber-600 transition-colors">
                    Analytics
                  </h3>
                ...
                >
                  ...
                  <h3 className="text-lg font-bold mb-1 group-hover:text-red-600 transition-colors">
                    Alerts
                  </h3>
            ...
            {recentActivity.length > 0 && (
              <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
                  ...
                  <button className="text-xs font-semibold text-green-600 hover:text-green-700 dark:text-green-400 uppercase tracking-wide">
                    View All
                  </button>
                </div>
                  ...
                  {recentActivity.map((activity) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <span
                          className={`text-sm font-bold ${activity.passed ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}
                        >
                          {activity.score}%
                        </span>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-emerald-900 to-teal-950 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
              ...
              <p className="text-green-100 text-sm relative z-10 leading-relaxed mb-4">
                Save time by using our AI Question Generator. Create
            ...
            <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              ...
              >
                ...
                <p className="text-gray-500 text-sm mb-4">
                  This action creates a permanent deletion request and cannot be
                  undone immediately.
                </p>
                <textarea
                  value={deleteReason}
