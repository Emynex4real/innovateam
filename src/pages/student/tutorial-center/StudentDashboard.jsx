...
const EnterpriseDashboard = () => {
  ...
  if (loading) {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${isDarkMode ? "bg-zinc-950" : "bg-gray-50"}`}
      >
  ...
  const MetricCard = ({ label, value, icon: Icon, trend }) => (
    ...
    >
      ...
      <div>
        ...
        <div
          className={`text-xs font-medium mt-1 ${isDarkMode ? "text-zinc-500" : "text-gray-500"}`}
        >
          {label}
        </div>
      </div>
  ...
  const ActionButton = ({ icon: Icon, label, desc, onClick }) => (
    ...
    >
      <div className="flex items-center gap-3 mb-2">
        <div
          className={`p-2 rounded-lg ${isDarkMode ? "bg-zinc-950 text-zinc-300" : "bg-gray-50 text-gray-700"} group-hover:scale-110 transition-transform`}
        >
          <Icon size={18} />
        </div>
        <span
  ...
  return (
    <div
      className={`min-h-screen ${isDarkMode ? "bg-zinc-950 text-zinc-100" : "bg-gray-50 text-gray-900"} font-sans`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          ...
          {primaryCenter && (
            <div className="flex items-center gap-4">
              <StreakBadge
                centerId={primaryCenter.center_id || primaryCenter.id}
              />
              <div
                className={`h-8 w-px ${isDarkMode ? "bg-zinc-800" : "bg-gray-200"}`}
              />
        ...
        {!primaryCenter ? (
          /* EMPTY STATE */
          <div
            className={`rounded-2xl border border-dashed p-12 text-center max-w-2xl mx-auto ${isDarkMode ? "border-zinc-800 bg-zinc-900/50" : "border-gray-300 bg-white"}`}
          >
            <div
              className={`inline-flex p-4 rounded-full mb-6 ${isDarkMode ? "bg-zinc-800 text-zinc-400" : "bg-gray-100 text-gray-500"}`}
            >
              <School size={32} />
            </div>
            <h3 className="text-xl font-bold mb-2">No Active Enrollment</h3>
            <p
              className={`mb-8 ${isDarkMode ? "text-zinc-400" : "text-gray-500"}`}
            >
              Join a tutorial center to access tests, resources, and track your
              progress.
            </p>
            <button
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 space-y-8">
              ...
              >
                ...
                <div className="p-6 md:p-8">
                  <div className="flex justify-between items-start mb-6">
                    <div>
                      <div
                        className={`inline-flex items-center gap-2 px-2.5 py-1 rounded-md text-xs font-semibold uppercase tracking-wide mb-3 ${isDarkMode ? "bg-zinc-800 text-zinc-300" : "bg-gray-100 text-gray-600"}`}
                      >
                        <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />{" "}
                        Active Center
                      </div>
                      <h2 className="text-2xl font-bold mb-1">
                        {primaryCenter.center_name || primaryCenter.name}
                      </h2>
                      <p
                        className={`font-mono text-sm ${isDarkMode ? "text-zinc-500" : "text-gray-500"}`}
                      >
                        ID: {primaryCenter.access_code}
                      </p>
                    </div>
                    {logoUrl && (
                      <img
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button
                      onClick={() =>
                        navigate(
                          `/student/tests?center=${primaryCenter.center_id || primaryCenter.id}`,
                        )
                      }
                    ...
                    <button
                      onClick={() =>
                        navigate(
                          `/student/analytics/${primaryCenter.center_id || primaryCenter.id}`,
                        )
                      }
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                />
                <MetricCard label="Global Rank" value="#--" icon={Trophy} />
              </div>
              ...
              {centers.length > 1 && (
                <div>
                  <h3
                    className={`text-sm font-bold uppercase tracking-wider mb-4 ${isDarkMode ? "text-zinc-500" : "text-gray-500"}`}
                  >
                    Other Enrollments
                  </h3>
                  <div className="space-y-3">
                    {centers.slice(1).map((center) => (
                      <div
                        ...
                        onClick={() =>
                          navigate(
                            `/student/tests?center=${center.center_id || center.id}`,
                          )
                        }
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(
                                `/student/analytics/${center.center_id || center.id}`,
                              );
                            }}
            ...
            <div className="lg:col-span-4 space-y-8">
              ...
              {/* Leaderboard Widget */}
              <div
                className={`rounded-xl border overflow-hidden ${isDarkMode ? "bg-zinc-900 border-zinc-800" : "bg-white border-gray-200"}`}
              >
                <div
                  className={`px-5 py-3 border-b flex justify-between items-center ${isDarkMode ? "border-zinc-800" : "border-gray-100"}`}
                >
                  <h3 className="font-semibold text-sm">Leaderboard</h3>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400">
                    LIVE
                  </span>
                </div>
                <div className="p-2">
              ...
              {/* Tip Widget */}
              <div
                className={`p-5 rounded-xl border ${isDarkMode ? "bg-green-950/30 border-green-900/50" : "bg-green-50 border-green-100"}`}
              >
                <div className="flex gap-3">
                  <Zap size={20} className="text-green-500 shrink-0 mt-0.5" />
                  <div>
                    <h4
                      className={`text-sm font-bold mb-1 ${isDarkMode ? "text-green-300" : "text-green-900"}`}
                    >
                      Did you know?
                    </h4>
                    <p
                      className={`text-xs leading-relaxed ${isDarkMode ? "text-green-400" : "text-green-700"}`}
                    >
                      Consistent practice is key. Students who take tests 3
                      times a week score 25% higher on average.
                    </p>
                  </div>
                </div>
              </div>
