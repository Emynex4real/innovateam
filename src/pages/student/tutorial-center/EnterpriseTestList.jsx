...
const EnterpriseTestList = () => {
  ...
  const filtered = tests.filter(
    (t) =>
      ...
      (filter === "all" ||
        (filter === "new" && !attempts[t.id]) ||
        (filter === "done" && attempts[t.id])),
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col gap-6 mb-8">
          ...
          >
            ...
            {/* Filter Tabs */}
            <div
              className={`flex p-1 rounded-xl w-full md:w-auto ${isDarkMode ? "bg-gray-950 border border-gray-800" : "bg-gray-100"}`}
            >
              {[
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div
              className={`p-4 rounded-full mb-4 ${isDarkMode ? "bg-gray-900" : "bg-gray-100"}`}
            >
                ...
                return (
                  <motion.div
                    ...
                    onClick={() =>
                      navigate(
                        test.is_cbt_mode
                          ? `/student/test/${test.id}/cbt`
                          : `/student/test/${test.id}`,
                      )
                    }
                  ...
                  >
                    ...
                    <div className="p-5 flex-1">
                      <div className="flex justify-between items-start mb-4">
                        <span
                          className={`px-2.5 py-1 rounded-md text-xs font-bold flex items-center gap-1.5 ${status.color}`}
                        >
                      ...
                      <h3
                        className={`text-lg font-bold mb-2 line-clamp-2 ${isDarkMode ? "text-white" : "text-gray-900"}`}
                      >
                        {test.title}
                      </h3>
                      <p
                        className={`text-sm line-clamp-2 mb-4 ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}
                      >
                      ...
                      <div className="flex flex-wrap gap-3">
                        <div
                          className={`flex items-center gap-1.5 text-xs px-2 py-1 rounded border ${isDarkMode ? "border-gray-800 bg-gray-950 text-gray-400" : "border-gray-100 bg-gray-50 text-gray-500"}`}
                        >
                          <Clock size={12} />
                          {test.time_limit}m
                        </div>
                        ...
                        {test.is_cbt_mode && (
                          <div className="flex items-center gap-1.5 text-xs px-2 py-1 rounded border bg-blue-50 border-blue-200 text-blue-700 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-300 font-bold">
                            <Monitor size={12} /> CBT Mode
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div
                      className={`px-5 py-4 border-t mt-auto flex items-center justify-between ${isDarkMode ? "border-gray-800" : "border-gray-100"}`}
                    >
                      {status.score !== undefined ? (
                        <div className="flex-1 mr-4">
                          ...
                          <div
                            className={`h-1.5 w-full rounded-full overflow-hidden ${isDarkMode ? "bg-gray-800" : "bg-gray-100"}`}
                          >
                            <div
