...
const EnterpriseTakeTest = () => {
    ...
    try {
      ...
      if (!accessResponse.canAttempt) {
        if (accessResponse.reason === "max_attempts_reached") {
          toast.error(
            `Maximum attempts (${accessResponse.maxAttempts}) reached`,
          );
        } else if (accessResponse.reason === "cooldown_active") {
          toast.error(
            `Please wait ${accessResponse.hoursRemaining} hour(s) before retrying`,
          );
        }
  ...
  const handleSubmit = async (autoSubmit = false) => {
    ...
    try {
      ...
      if (response.success) {
        console.log(
          "✅ Test submitted successfully, attempt_id:",
          response.attempt_id,
        );
  ...
  if (loading) {
    return (
      <div
        className={`min-h-screen flex flex-col items-center justify-center ${isDarkMode ? "bg-zinc-950 text-zinc-400" : "bg-gray-50 text-gray-500"}`}
      >
  ...
  return (
    <div
      className={`min-h-screen flex flex-col font-sans ${isDarkMode ? "bg-zinc-950 text-zinc-100" : "bg-gray-50 text-gray-900"}`}
    >
      {/* Proctoring Monitor */}
      ...
      {/* 1. TOP CONTROL BAR */}
      <header
        className={`sticky top-0 z-20 border-b backdrop-blur-md ${isDarkMode ? "bg-zinc-900/90 border-zinc-800" : "bg-white/90 border-gray-200"}`}
      >
        {/* Progress Line */}
        ...
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-mono text-sm font-bold tabular-nums transition-colors ${timerColor}`}
          >
      ...
      >
        <AnimatePresence mode="wait">
          ...
          >
            {/* Question Text */}
            <div
              className={`text-lg md:text-xl font-medium leading-relaxed ${isDarkMode ? "text-zinc-100" : "text-gray-900"}`}
            >
              {/* Question Image */}
              {currentQuestion.image_url && (
                <div className="mb-4">
                  <img
                    ...
                    className={`max-w-full max-h-[300px] rounded-lg border mx-auto object-contain ${isDarkMode ? "border-zinc-700" : "border-gray-200"}`}
                  />
                </div>
              )}
              <div className="flex gap-4">
                <span
                  className={`flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg text-sm font-bold ${isDarkMode ? "bg-zinc-800 text-zinc-400" : "bg-gray-100 text-gray-500"}`}
                >
                  {currentIdx + 1}
                </span>
                <div className="pt-1">
              ...
              {["A", "B", "C", "D"].map((letter, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`flex-1 text-base ${isSelected ? "font-semibold text-green-900 dark:text-green-100" : isDarkMode ? "text-zinc-300" : "text-gray-700"}`}
                    >
                      <MathText text={currentQuestion.options[idx]} />
                    </div>

                    <span
                      className={`absolute top-4 right-4 text-xs font-bold opacity-0 transition-opacity ${isSelected ? "opacity-100 text-green-600 dark:text-green-400" : ""}`}
                    >
                      {letter}
                    </span>
                  </button>
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 p-4 border-t z-20 ${isDarkMode ? "bg-zinc-900 border-zinc-800" : "bg-white border-gray-200"}`}
      >
        <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
          ...
          {currentIdx === test.questions.length - 1 ? (
            ...
            >
              ...
              <span className="sm:hidden">Next</span> <ChevronRight size={20} />
            </button>
          )}
        </div>
      </div>
            ...
            >
              ...
              <div className="grid grid-cols-5 gap-3 overflow-y-auto p-1 flex-1">
                {test.questions.map((q, i) => {
                  ...
                  if (isActive)
                    statusClass =
                      "bg-green-600 text-white border-green-600 ring-2 ring-green-300 dark:ring-green-900";
                  ...
                  return (
                    ...
                    >
                      {i + 1}
                      {isFlag && (
                        <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-amber-500 rounded-full border border-white dark:border-zinc-900" />
                      )}
                    </button>
                  );
                })}
              ...
              <div
                className={`mt-6 pt-6 border-t space-y-3 ${isDarkMode ? "border-zinc-800" : "border-gray-100"}`}
              >
                <div className="flex justify-between text-xs font-medium text-gray-500">
                  ...
                  <span className="flex items-center gap-1.5">
                    <span
                      className={`w-2 h-2 rounded-full ${isDarkMode ? "bg-green-400" : "bg-green-100"}`}
                    ></span>{" "}
                    Answered
                  </span>
                  <span className="flex items-center gap-1.5">
