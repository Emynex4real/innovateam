...
const tutorialCenterService = {
  ...
  async getLeaderboard(questionSetId, filter = "all") {
    try {
      ...
      const { data: attempts, error } = await query.order("score", {
        ascending: false,
      });
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          difficulty_level:
            questionData.difficulty_level || questionData.difficulty,
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          is_cbt_mode: setData.is_cbt_mode || false,
          exam_type: setData.exam_type || "custom",
          shuffle_questions: setData.shuffle_questions || false,
          enable_lockdown: setData.enable_lockdown || false,
        })
        .select()
        .single();
