...
const messagingService = {
    ...
    try {
      ...
      if (conversationId && !receiverId) {
        ...
        if (!convError && conv) {
          ...
          receiverId =
            conv.participant_1_id === senderId
      ...
      if (!conversationId) {
        if (!receiverId)
          throw new Error("Cannot send message: Receiver ID missing");
        ...
        const { data: existingConv } = await supabase
          ...
          .select("id")
          .or(
            `and(participant_1_id.eq.${senderId},participant_2_id.eq.${receiverId}),and(participant_1_id.eq.${receiverId},participant_2_id.eq.${senderId})`,
          )
          .maybeSingle();
      ...
      // 6. Invalidate receiver's and sender's unread count and conversation caches
      await cacheService.del(`unread_msgs:${receiverId}`);
      await cacheService.del(`conversations:${receiverId}`);
      await cacheService.del(`conversations:${senderId}`);
  ...
  async getConversations(userId) {
    ...
    const cachedConversations = await cacheService.get(cacheKey);
    ...
    // Cache conversations for 5 minutes (invalidates on new message)
    await cacheService.set(cacheKey, formattedConversations, 300);
  ...
  async getMessages(userId, conversationId, limit = 50) {
    ...
    // If any messages were marked as read, invalidate the unread and conversations cache
    if (updatedMessages && updatedMessages.length > 0) {
      await cacheService.del(`unread_msgs:${userId}`);
  ...
  // 3b. Get lightweight unread count
  async getUnreadCount(userId) {
    const cacheKey = `unread_msgs:${userId}`;
    const cachedCount = await cacheService.get(cacheKey);
    if (cachedCount !== null) {
      return { success: true, unreadCount: Number(cachedCount) };
    }

    const { data, error } = await supabase
      .from("conversations")
      .select(
        "participant_1_id, participant_2_id, participant_1_unread, participant_2_unread",
      )
      .or(`participant_1_id.eq.${userId},participant_2_id.eq.${userId}`);
    ...
    if (data) {
      for (const conv of data) {
        if (conv.participant_1_id === userId)
          unreadCount += conv.participant_1_unread || 0;
        else if (conv.participant_2_id === userId)
          unreadCount += conv.participant_2_unread || 0;
      }
    }

    // Cache the fast count for 60 seconds
