...
class ForumService {
  async getCategories(centerId) {
    try {
      ...
      const cacheKey = `forum:categories:${centerId}`;
      const cached = await cache.get(cacheKey);
      if (cached) return { success: true, data: cached, count: cached.length };
      ...
      await cache.set(cacheKey, data || [], 300); // Cache for 5 mins
  ...
  async getThreads(categoryId, page = 1, limit = 20) {
    try {
      ...
      const cacheKey = `forum:threads:${categoryId}:${page}:${limit}`;
      const cached = await cache.get(cacheKey);
      if (cached)
        return {
          success: true,
          data: cached,
          pagination: { page, limit, hasMore: cached.length === limit },
        };
  ...
  async getThread(threadId, userId) {
    try {
      ...
      return {
        success: true,
        data: { ...thread, posts: rootPosts, is_following: isFollowing },
      };
    } catch (error) {
  ...
  async votePost(postId, userId, voteType) {
    try {
      ...
      return {
        success: true,
        upvotes: updated?.upvote_count || 0,
        downvotes: updated?.downvote_count || 0,
      };
    } catch (error) {
