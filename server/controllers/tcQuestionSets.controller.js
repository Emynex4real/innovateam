...
exports.getQuestionSets = async (req, res) => {
  try {
    ...
    if (center) {
      ...
      query = query.eq("center_id", center.id).is("student_id", null);
    } else {
      ...
      query = query.eq("is_active", true).in("center_id", enrolledCenterIds);
    ...
    if (!center) {
      filteredData =
        data?.filter((test) => {
          return !test.student_id || test.student_id === userId;
        }) || [];
    }
