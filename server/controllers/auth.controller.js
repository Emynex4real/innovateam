...
const { hashPassword, verifyPassword, validatePasswordStrength } = require('../utils/passwordSecurity');
const { logSecurityEvent, SECURITY_EVENTS, SEVERITY } = require('../utils/auditLogger');
const { validateEmail, validateName, sanitizeObject } = require('../utils/inputValidation');
...
exports.register = async (req, res) => {
  try {
    ...
    const { name, email, phoneNumber, password } = sanitized;
    ...
    if (error) {
      ...
      await logSecurityEvent({
        ...
        ipAddress: req.ip,
        details: { reason: error.message, email: emailValidation.value },
        success: false
      });
  ...
  try {
    ...
    if (error || !data.user) {
      ...
      await logSecurityEvent({
        ...
        ipAddress: req.ip,
        userAgent: req.headers['user-agent'],
        details: { email: emailValidation.value, reason: error?.message },
        success: false
      });
