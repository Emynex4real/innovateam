...
const {
  ...
  GamificationService,
} = require("../services/notificationsGamificationService");
...
// Get unread count
router.get("/messaging/unread-count", authenticate, async (req, res) => {
  try {
...
// Mark post as answer
router.post(
  "/forums/posts/:postId/mark-answer",
  authenticate,
  async (req, res) => {
    try {
      const { postId } = req.params;
      const { threadId } = req.body;
      const result = await forumsService.markAsAnswer(
        postId,
        threadId,
        req.user.id,
      );
...
router.post(
  "/forums/threads/:threadId/follow",
  authenticate,
  async (req, res) => {
    try {
      const { threadId } = req.params;
...
router.post(
  "/forums/threads/:threadId/unfollow",
  authenticate,
  async (req, res) => {
    try {
      const { threadId } = req.params;
...
router.get("/study-groups/all/explore", authenticate, async (req, res) => {
  ...
  const result = await studyGroupsService.getGroups(
    null,
    req.user.id,
    parseInt(page),
    parseInt(limit),
...
router.get("/study-groups/search/:centerId", authenticate, async (req, res) => {
  const { centerId } = req.params;
  const { q, page = 1, limit = 20 } = req.query;
  const result = await studyGroupsService.searchGroups(
    centerId,
    q,
    parseInt(page),
    parseInt(limit),
...
router.post("/study-groups/:groupId/posts", authenticate, async (req, res) => {
  ...
  const result = await studyGroupsService.postInGroup(
    groupId,
    req.user.id,
    content,
    attachmentUrl,
...
router.get("/tutoring/tutors/:centerId", authenticate, async (req, res) => {
  ...
  const result = await peerTutoringService.getTutors(
    centerId,
    subject,
    parseInt(page),
    parseInt(limit),
...
// Get tutor profile
router.get(
  "/tutoring/tutors/:tutorId/profile",
  authenticate,
  async (req, res) => {
...
router.post("/tutoring/profile", authenticate, async (req, res) => {
  ...
  const result = await peerTutoringService.createTutorProfile(
    req.user.id,
    centerId,
    bio,
    hourlyRate,
...
router.post("/tutoring/requests", authenticate, async (req, res) => {
  ...
  const result = await peerTutoringService.requestTutoring(
    req.user.id,
    tutorId,
    centerId,
    subject,
...
router.post(
  ...
  async (req, res) => {
    ...
    const result = await peerTutoringService.acceptTutoringRequest(
      requestId,
      req.user.id,
    );
    res.json(result);
...
router.post(
  ...
  async (req, res) => {
    const { requestId } = req.params;
    const result = await peerTutoringService.declineTutoringRequest(
      requestId,
      req.user.id,
    );
    res.json(result);
...
router.post("/tutoring/sessions", authenticate, async (req, res) => {
  ...
  const result = await peerTutoringService.scheduleSession(
    requestId,
    scheduledAt,
    meetingLink,
  );
...
// Complete session
router.post(
  "/tutoring/sessions/:sessionId/complete",
  authenticate,
  async (req, res) => {
    const { sessionId } = req.params;
    const { rating, feedback } = req.body;
    const result = await peerTutoringService.completeSession(
      sessionId,
      req.user.id,
      rating,
      feedback,
...
// Get leaderboard
router.get(
  "/gamification/leaderboard/:centerId",
  authenticate,
  async (req, res) => {
    const { centerId } = req.params;
...
router.get(
  "/gamification/achievements/:centerId",
  authenticate,
  async (req, res) => {
    const { centerId } = req.params;
    const result = await GamificationService.getAchievementsSummary(
      req.user.id,
      centerId,
    );
    res.json(result);
...
router.put("/forums/posts/:postId", authenticate, async (req, res) => {
  try {
    ...
    if (!content?.trim() || content.length < 5) {
      return res
        .status(400)
        .json({ success: false, error: "Post must be at least 5 characters" });
    }
